...
class GraspNode(Node):
    def __init__(self):
        ...
        self.fail_state = self.create_publisher(Bool, '/fail_state', 10)
    ...
    def pc_callback(self, pc_msg):
        ...
        if self._attempts >= 3:
            self.get_logger().warn("Fail state active — grasping halted.", throttle_duration_sec=5.0)
            return
    ...
    def grasp_response_callback(self, future):
        ...
        try:
            ...
            if response.success:
                ...
                if current_count < self._baseline_count:
                    self.get_logger().info("Centroid count reduced — grasp confirmed.")
                    self._baseline_count -= 1
                    self._attempts = 0
                else:
                    self.get_logger().warn("Centroid count unchanged — grasp unconfirmed.")
                    self._attempts += 1
            else:
            ...
            if self._attempts >= 3:
                self.get_logger().warn("WARNING: No improvement after 3 attempts, entering fail_state.")
                fail_msg.data = True

            self.fail_state.publish(fail_msg)

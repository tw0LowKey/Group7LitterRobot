...
class LitterHandlerNode(Node):
    ...
    Publishes:
        ...
        /queue_size              — current size of the litter queue
    ...
    def __init__(self):
        super().__init__("litter_handler_node_new")
        ...
        self.declare_parameter("monitoring_enabled", "/litter_monitor_enable")
        ...
        self.monitor_enabled = self.get_parameter("monitoring_enabled").value
        ...
        self.monitor_enable_pub = self.create_publisher(
            Bool,
    ...
    def publish_queue_size(self):
        msg = Int32()
        with self.lock:
            msg.data = len(self.litter_queue) + (1 if self.current_litter_pose is not None else 0)
    ...
    def start_nav_callback(self, msg: Bool):
        if msg.data and self.state == self.STATE_WAITING_PICKUP:
            ...
            self.publish_queue_size()
            self.monitor_enable_pub.publish(Bool(data=True))
    ...
    def rotate_response_callback(self, future):
        ...
        if not goal_handle.accepted:
            ...
            self.monitor_enable_pub.publish(Bool(data=False))
            return

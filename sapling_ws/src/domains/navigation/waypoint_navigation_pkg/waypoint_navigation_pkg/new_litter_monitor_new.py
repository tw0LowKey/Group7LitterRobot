...
class PointCloudMonitor(Node):
    def __init__(self):
        super().__init__('new_litter_monitor_new')
        ...
        self.enable_sub = self.create_subscription(
            Bool, '/litter_monitor_enable', self.enable_cb, 10
        )

        # Publishers
        self.pause_pub = self.create_publisher(
            Bool, '/pause_navigation', 10
        )
    ...
    def enable_cb(self, msg):
        self.monitoring_enabled = msg.data
        state = "enabled" if msg.data else "disabled"
        self.get_logger().info(f'Litter monitoring {state}.')

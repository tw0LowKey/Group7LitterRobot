...
class LitterHandlerNode(Node):
    ...
    Subscribes:
        ...
        /leader/travel_heading    — heading along waypoint path (rad)
    ...
    States:
        ...
        NAVIGATING        — driving to litter pose (Phase 1)
        ROTATING          — rotating to face litter (Phase 2)
        ALIGNING          — rotating to face travel direction (Phase 3)
    ...
    def __init__(self):
        ...
        self.travel_heading_sub = self.create_subscription(
            Float64,
            self.travel_heading_topic,
            self.travel_heading_callback,
            10,
            callback_group=self.cb_group,
        )
    ...
    # ============================================================
    # Travel heading callback
    # ============================================================

    def travel_heading_callback(self, msg: Float64):
        self.travel_heading = msg.data
    ...
    def litter_callback(self, msg: PoseStamped):
        ...
        if self.state == self.STATE_IDLE:
            ...
            # Nav2 doesn't cancel instantaneously — if we send a new goal while
            # the old one is still being cancelled, it gets rejected.  The settle
            # timer gives Nav2 a moment to finish the cancellation cleanly.
    ...
    def get_approach_pose(self, litter_pose: PoseStamped):
        ...
        # We stop short of the litter rather than driving right on top of it —
        # the follower needs to pull up alongside, and we want a clean gap.
    ...
    def get_align_pose(self):
        """Goal at current position but rotated to face the travel heading."""
        # After picking up litter the robot is facing sideways.  Re-aligning to
        # the sweep direction before resuming means Nav2 doesn't have to spin
        # in place right as it picks up the next waypoint.
        if self.current_pose is None or self.travel_heading is None:
            return None

        heading = self.travel_heading

        align_pose = PoseStamped()
        align_pose.header.frame_id = self.goal_frame
        align_pose.header.stamp = self.get_clock().now().to_msg()
        align_pose.pose.position.x = self.current_pose.position.x
        align_pose.pose.position.y = self.current_pose.position.y
        align_pose.pose.position.z = 0.0
        align_pose.pose.orientation.z = math.sin(heading / 2.0)
        align_pose.pose.orientation.w = math.cos(heading / 2.0)
    ...
    # ============================================================
    # Goal sending — Phase 3: rotate to face travel direction
    # ============================================================

    def send_align_goal(self):
        align_pose = self.get_align_pose()

        if align_pose is None:
            self.get_logger().warn(
                "No travel heading available — skipping alignment, calling bin."
            )
            with self.lock:
        ...
        goal_msg = NavigateToPose.Goal()
        goal_msg.pose = align_pose

        self.get_logger().info(
            f"[Litter Phase 3] Aligning to travel heading "
            f"({math.degrees(self.travel_heading):.1f}°)"
        )
        ...
        future.add_done_callback(self.align_response_callback)
    ...
    def approach_result_callback(self, future):
        ...
        with self.lock:
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                ...
                # Even if we couldn't reach the ideal approach spot (obstacle,
                # narrow gap), we still rotate to face the litter so the
                # follower knows exactly where to pull up.
                self.state = self.STATE_ROTATING
    ...
    # ============================================================
    # Phase 2 callbacks (rotate to face litter)
    # ============================================================
    ...
    def rotate_response_callback(self, future):
        ...
        if not goal_handle.accepted:
            self.get_logger().warn("Rotate goal rejected — moving to alignment.")
            with self.lock:
                self.is_navigating = False
    ...
    def rotate_result_callback(self, future):
        ...
        with self.lock:
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                self.get_logger().info(
                    "Phase 2 complete — facing litter. Aligning to travel heading."
                )
                self.state = self.STATE_ALIGNING

            elif status == GoalStatus.STATUS_ABORTED:
                self.get_logger().warn(
                    "Rotate aborted — moving to alignment."
                )
                self.state = self.STATE_ALIGNING
    ...
    # ============================================================
    # Phase 3 callbacks (align to travel heading)
    # ============================================================
    ...
    def align_response_callback(self, future):
        ...
        if goal_handle is None:
            self.get_logger().warn("Failed to get align goal handle.")
            with self.lock:
                self.is_navigating = False
                self.state = self.STATE_WAITING_PICKUP
            self.publish_call_bin()
        ...
        if not goal_handle.accepted:
            self.get_logger().warn("Align goal rejected — calling bin anyway.")
            with self.lock:
                self.is_navigating = False
                self.state = self.STATE_WAITING_PICKUP
            self.publish_call_bin()
        ...
        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(self.align_result_callback)

    def align_result_callback(self, future):
        result = future.result()

        if result is None:
            self.get_logger().warn("No align result received.")
            with self.lock:
                self.is_navigating = False
                self.state = self.STATE_WAITING_PICKUP
            self.publish_call_bin()
        ...
        with self.lock:
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                self.get_logger().info(
                    "Phase 3 complete — aligned to travel heading. Calling bin."
                )
                self.state = self.STATE_WAITING_PICKUP
                self.publish_call_bin()
                ...
                self.get_logger().warn(
                    "Align aborted — calling bin at current heading."
                )
                self.state = self.STATE_WAITING_PICKUP
                self.publish_call_bin()

            elif status == GoalStatus.STATUS_CANCELED:
                self.get_logger().info("Align goal cancelled.")

...
class FollowerCoordinationNode(Node):
    ...
    def __init__(self):
        ...
        # The follower broadcasts its status continuously, so when a new call_bin
        # arrives the most recent message might still say parked=True from the
        # previous pickup.  We require seeing parked=False after the call_bin
        # is set before accepting any parked=True — that way we know the follower
        # has actually moved and freshly parked for this specific litter pick.
        self.call_bin_seq = 0
    ...
    def sweep_done_callback(self, msg: Bool):
        if msg.data:
            ...
            # When the leader finishes its whole sweep, we still want the
            # follower to park for a final collection — same call_bin mechanism,
            # just triggered by sweep completion rather than litter detection.
            self.get_logger().info(
    ...
    def follower_status_callback(self, msg: FollowerStatus):
        ...
        with self.lock:
            # Watch for the parked=False → parked=True transition.  Without this
            # check, a stale parked=True from the previous litter pickup would
            # immediately fire the timer for the new one before the follower
            # has even started moving.
            if not msg.parked:
                self.last_follower_parked_false_seen = True

            # Only start the pickup timer once we're confident the follower has
            # freshly parked for the current call (all four conditions must hold).
            if (msg.parked

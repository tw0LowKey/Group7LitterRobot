...
class WaypointGeneratorNode(Node):
    ...
    def generate_lawnmower_gps(self, top_left_lat, top_left_lon,
        ...
        # Pull each row 0.5 m away from the east and west edges so the robot
        # doesn't have to squeeze against the boundary when turning at the end
        # of a row — gives it room to arc around cleanly.
    ...
    def area_coord_callback(self, request, response):
        ...
        self.get_logger().info(
            f"Corner A: ({request.top_left_latitude:.8f}, "
            f"{request.top_left_longitude:.8f})"
        )
        self.get_logger().info(
            f"Corner B: ({request.bottom_right_latitude:.8f}, "
        ...
        # Auto-correct corner order — user can provide corners in any order
        north_lat = max(request.top_left_latitude, request.bottom_right_latitude)
        south_lat = min(request.top_left_latitude, request.bottom_right_latitude)
        west_lon = min(request.top_left_longitude, request.bottom_right_longitude)
        east_lon = max(request.top_left_longitude, request.bottom_right_longitude)

        self.get_logger().info(
            f"Sorted area: NW ({north_lat:.8f}, {west_lon:.8f}) "
            f"SE ({south_lat:.8f}, {east_lon:.8f})"
        )
        ...
        for i, (lat, lon) in enumerate(gps_waypoints):
            ...
            if i < len(gps_waypoints) - 1:
                ...
                # At the end of a row, the very next waypoint is directly
                # north or south (the inter-row step), not along the sweep
                # direction.  If we used that for heading the robot would
                # face the wrong way entering the new row.  Instead we look
                # one waypoint further ahead to get the actual row direction.

...
class GPSFollowerBehaviorNode(Node):
    ...
    def gps_xy_to_odom(self, gps_x, gps_y):
        ...
        """
        # GPS-XY and odom both measure position in metres but their origins are
        # different — GPS-XY uses the first GPS fix as (0,0), while odom uses
        # wherever the robot was when it booted.  We figure out the constant
        # shift between them by comparing the follower's own position in both
        # frames, then apply that shift to any GPS-XY point we want Nav2 to
        # navigate to (Nav2 only understands odom/map coordinates).
    ...
    def leader_pose_callback(self, msg: LeaderPose):
        ...
        # Handle call_bin while mid-parking
        if msg.call_bin and self.state == self.STATE_PARKING:
            return
        ...
        if dist >= self.breadcrumb_distance:
            with self.lock:
                # Store breadcrumbs in odom frame so they can be sent directly
                # to Nav2 without any further conversion at goal-send time.
    ...
    def tick_parking(self):
        """Drive to park_distance directly behind leader using leader's heading."""
        ...
        # Total physical gap between the two robot centres when parked:
        # half of leader body + desired gap + half of follower body.
        total_offset = (
        ...
        # Walk backwards along the leader's heading by total_offset to find the
        # spot directly behind it — adding π flips the direction 180 degrees.
        ...
        self.get_logger().info(
            f"[PARKING] Leader odom ({leader_odom_x:.2f}, {leader_odom_y:.2f}), "
    ...
    def tick_waiting(self):
        ...
        if dist_to_leader > self.resume_distance:
            ...
            with self.lock:
                self.state = self.STATE_FOLLOWING
                # Clear the old breadcrumb trail — those positions are behind
                # the leader now and following them would make the robot double
                # back.  We start a fresh trail from the leader's current spot.
    ...
    def park_result_callback(self, future):
        ...
        with self.lock:
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                ...
                self.get_logger().warn("Park goal aborted — parking here.")
                self.state = self.STATE_WAITING
                self.set_bin_ready(True)
                self.parked_for_current_call = True
